...
export default function Header({ isOnePage = false, sectionRefs = {} }) {
  ...
  const handleNavigation = (id, path) => {
    ...
    if (isOnePage && sectionRefs[id]) {
      // One-page mode: smooth scroll to section
      sectionRefs[id].current?.scrollIntoView({
        behavior: 'smooth',
        block: 'start'
      });
    }
    // If not one-page mode, React Router will handle navigation
  };
            ...
            <div className={`${isMenuOpen ? 'block' : 'hidden'} md:block`}>
              <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-1 lg:gap-2 py-4 md:py-0">
                {navLinks.map(({ id, icon: Icon, text, path }) => (
                  <div key={id}>
                    {isOnePage ? (
                      // One-page mode: use button for smooth scrolling
                      <button
                      ...
                      // Router mode: use Link for navigation
                      <Link
                        to={path}
                        onClick={() => handleNavigation(id, path)}
                        className={`px-3 py-2 md:py-1.5 rounded-lg md:rounded-full text-sm font-medium
